...
class MyExplorer(Node):
    ...
    def scan_callback(self, msg):
        min_dist = min([r for r in msg.ranges if not np.isinf(r) and not np.isnan(r)])
        self.get_logger().info(f"Minimum distance from Lidar: {min_dist:.2f}m at angle {msg.ranges.index(min_dist)} degrees")
        time.sleep(5)
